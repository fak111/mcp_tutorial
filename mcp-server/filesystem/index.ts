  ...
  try {
    ...
    switch (name) {
      ...
      case "read_text_file": {
        ...
        const validPath = validatePath(parsed.data.path, allowedDirectories);
      ...
      case "read_multiple_files": {
        ...
        const results = await Promise.all(
          parsed.data.paths.map(async (filePath: string) => {
            try {
              const validPath = validatePath(filePath, allowedDirectories);
              const content = await readFileContent(validPath);
              return `${filePath}:\n${content}\n`;
            } catch (error) {
      ...
      case "edit_file": {
        ...
        const validPath = validatePath(parsed.data.path, allowedDirectories);
        const result = await applyFileEdits(validPath, parsed.data.edits, allowedDirectories);
        return {
      ...
      case "directory_tree": {
        ...
        async function buildTree(currentPath: string, excludePatterns: string[] = []): Promise<TreeEntry[]> {
          const validPath = validatePath(currentPath, allowedDirectories);
          const entries = await fs.readdir(validPath, { withFileTypes: true });
      ...
      case "move_file": {
        ...
        const validSourcePath = validatePath(parsed.data.source, allowedDirectories);
        const validDestPath = validatePath(parsed.data.destination, allowedDirectories);
      ...
      case "search_files": {
        ...
        const validPath = validatePath(parsed.data.path, allowedDirectories);
        const results = await searchFilesWithValidation(parsed.data.pattern, allowedDirectories, 100);
        return {
...
async function updateAllowedDirectoriesFromRoots(requestedRoots: Root[]) {
  const rootPaths = requestedRoots.map(root => root.uri);
  const validatedRootDirs = await getValidRootDirectories(rootPaths);
